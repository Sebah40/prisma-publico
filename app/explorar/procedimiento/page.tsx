          ...
          <tbody className="divide-y divide-border">
            {data.map((c, i) => (
              <tr key={`${c.numero_procedimiento}-${i}`} className="hover:bg-surface transition-colors">
                ...
                <td className="px-3 py-2 text-[11px] text-text-secondary max-w-48 truncate">
                  {(c.saf_desc || "").replace(/^\d+\s*-\s*/, "")}
                </td>
                <td className="px-3 py-2 text-[11px]">
                  <Link href={`/identidades/${c.cuit_proveedor}`} className="text-mint">{c.proveedor_desc}</Link>
                </td>
                <td className="px-3 py-2 text-right font-data text-[11px] text-text-secondary">{formatPesos(c.monto)}</td>
                <td className="px-3 py-2 text-right font-data text-[11px] text-white">{formatPesos(c.monto_ajustado)}</td>
                <td className="px-3 py-2 text-right font-data text-[11px] text-text-secondary">{c.ejercicio}</td>
              </tr>
            ))}
          </tbody>
        </table>
