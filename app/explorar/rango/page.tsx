      ...
      <div className="overflow-x-auto">
        <table className="w-full border-collapse text-left">
          <thead className="border-b border-border text-[10px] uppercase tracking-wider text-text-secondary">
            <tr>
              <th className="hidden sm:table-cell px-3 py-2 font-medium">CUIT</th>
              ...
              <th className="hidden sm:table-cell px-3 py-2 font-medium text-right">Organismos</th>
            </tr>
          </thead>
          <tbody className="divide-y divide-border">
            {data.map((p) => (
              <tr key={p.cuit} className="hover:bg-surface transition-colors">
                <td className="hidden sm:table-cell px-3 py-2 font-data text-xs">
                  <Link href={`/identidades/${p.cuit}`} className="text-mint">{p.cuit}</Link>
                </td>
                <td className="px-3 py-2 text-xs text-white">{p.razon_social}</td>
                <td className="px-3 py-2 text-right font-data text-xs text-white">{formatPesos(p.monto_promedio)}</td>
                <td className="px-3 py-2 text-right font-data text-xs text-text-secondary">{formatPesos(p.total_adjudicado)}</td>
                <td className="px-3 py-2 text-right font-data text-xs text-text-secondary">{p.cantidad_contratos}</td>
                <td className="hidden sm:table-cell px-3 py-2 text-right font-data text-xs text-text-secondary">{p.jurisdicciones_distintas}</td>
              </tr>
            ))}
          </tbody>
        </table>
